...
- 只使用 packet 中正式提供的 speaker/addressee/relationship/scene 与 resolved constraints；缺少受话人或关系时不能仅凭 speaker 猜敬语等级，但缺少字段本身也不是错误。源文已明确语用功能、候选又避开未知关系特定形式时，应检查实际表达，不自动要求待确认。
- 源文礼貌标记首先表示请求、缓和或尊重，不自动等于目标语言的敬语等级或特定句尾。分别检查语用力度和目标语社会语域，避免把敌对、亲密或强制语气机械软化成礼貌体。
- 口吻连续性只能依据已验证的 scene/group、关系、场景语气或 resolved constraint；没有边界的相邻句不能证明同一场景。SG 的句尾规则必须精确适用于当前文本类型，不得从相似标签类推。
